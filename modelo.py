...
def carregar_dados():
    ...
    df_doencas_graves = pd.read_csv("dados/doencas_caninas_eutanasia_expandidas.csv")
    df_doencas_comuns = pd.read_csv("dados/top100_doencas_caninas.csv")
    return df, df_doencas_graves, df_doencas_comuns

def treinar_modelos(df, df_doencas_graves, df_doencas_comuns, features, features_eutanasia):
    le_mob = LabelEncoder()
    le_app = LabelEncoder()
    ...
    palavras_chave = [normalizar_texto(d) for d in df_doencas_graves['Doenca'].dropna().unique()]
    df['Doenca'] = df['Doenca'].fillna("").apply(normalizar_texto)
    df['tem_doenca_letal'] = df['Doenca'].apply(lambda d: int(any(p in d for p in palavras_chave)))
...
def prever(anamnese, modelos, le_mob, le_app, palavras_chave, features, features_eutanasia):
    ...
    sintomas_terminais = any(p in texto_norm for p in [
        ...
        "nao reage", "vomitos frequentes", "febre alta", "sem resposta", "sem reacoes"
    ])
    ...
    dias = 0
    if internar:
        dias_base = modelo_dias.predict(entrada[features])[0]
