...
def extrair_doencas_graves(texto_norm, palavras_chave):
    doencas_encontradas = [d for d in palavras_chave if d in texto_norm]
    tem_doenca_letal = int(len(doencas_encontradas) > 0)
    return doencas_encontradas, tem_doenca_letal
...
def prever(anamnese, modelos, le_mob, le_app, palavras_chave, features, features_eutanasia):
    ...
    doencas_detectadas, tem_doenca_letal = extrair_doencas_graves(texto_norm, palavras_chave)
    ...
    return {
        ...
        "Doenças Detectadas": doencas_detectadas if doencas_detectadas else ["Nenhuma"],
        "Doenças Graves Detectadas": [d for d in doencas_detectadas if d in palavras_chave] or ["Nenhuma grave"]
    }
